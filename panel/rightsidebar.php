...
<aside class="rSidebar">
	...
	<form id="profile_form" enctype="multipart/form-data"> 
		...
		<h4>Kullanıcı Ayarları</h4>
			...
			<?php if ($userDB['ucretli'] == 1) { ?>
			<div class="user_stat ceza-kesim">
				...
				<div>
					...
					<?php 
					$ceviri_sorgu = "SELECT * FROM bolumler WHERE cevirmen = 131 AND ceviri_teslim >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY ceviri_teslim DESC LIMIT 4";
					$clean_sorgu = "SELECT * FROM bolumler WHERE cleaner = 131 AND clean_teslim >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY clean_teslim DESC LIMIT 4";
					$typeset_sorgu = "SELECT * FROM bolumler WHERE typesetter = 131 AND typeset_teslim >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY typeset_teslim DESC LIMIT 4";
					$kontrol_sorgu = "SELECT * FROM bolumler WHERE kontrolcu = 131 AND kontrol_teslim >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY kontrol_teslim DESC LIMIT 4";

					$sonuc_array = [];

					foreach ([$ceviri_sorgu, $clean_sorgu, $typeset_sorgu, $kontrol_sorgu] as $sorgu) {
					    $sonuclar = $database->query($sorgu);
					    while ($sonuc = $sonuclar->fetch_assoc()) {
					        $sonuc_array[] = $sonuc;
					    }
					}

					// Tüm sonuçları tarihe göre sırala
					usort($sonuc_array, function($a, $b) {
					    return strtotime($a['teslim_tarihi']) - strtotime($b['teslim_tarihi']);
					});

					$son_dort_bolum = array_slice($sonuc_array, -4);


					/*if ($son_dort_sonuclar->num_rows == 4) {
						
					    $kullanici_sutunlari = array();
					    if ($sondanDort['cevirmen'] == $userDBID) {
					        $kullanici_sutunlari[] = 'ceviri_teslim';
					    }
					    if ($sondanDort['cleaner'] == $userDBID) {
					        $kullanici_sutunlari[] = 'clean_teslim';
					    }
					    if ($sondanDort['typesetter'] == $userDBID) {
					        $kullanici_sutunlari[] = 'typeset_teslim';
					    }
					    if ($sondanDort['kontrolcu'] == $userDBID) {
					        $kullanici_sutunlari[] = 'kontrol_teslim';
					    }
					    $en_eski_tarih = new DateTime('+100 years'); // Büyük bir tarih ile başlayalım
					    foreach ($kullanici_sutunlari as $sutun) {
					        $tarih = new DateTime($sondanDort[$sutun]);
					        if ($tarih < $en_eski_tarih) {
					            $en_eski_tarih = $tarih;
					        }
					    }
						$sonTarih = $en_eski_tarih;
					}
					else 
						$sonTarih = new DateTime('-1 week');

					$sonTarih->add(new DateInterval('P1W'));
					$cikti = $sonTarih->format('d/m l');*/
					//echo
					//str_replace(["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday","Sunday"],
    				//			["Pzt", "Salı", "Çrş", "Perş", "Cuma", "Cmt", "Pazar"], $cikti);
					?>
				</div>
			</div>
			<span style="font-style: italic;text-align: center;width: 100%;font-size: 11px;margin-bottom: 10px;color: gray;font-weight: bold;padding: 0 10px;box-sizing: border-box;">
				(Eğer gönüllüyseniz, son 1 hafta içinde ceza yediyseniz veya izniniz yeni bitti ise düzgün çalışmayabilir.)
			</span>
			<?php } ?>
			...
			<div class="user_stat">
				...
				<div class="user_statvalue">
					...
					<?php 
						...
						$redakteler = $database->query("SELECT * FROM bolumler WHERE redaktor = '$userDBID'");
						...
						$kontroller = $database->query("SELECT * FROM bolumler WHERE kontrolcu = '$userDBID'");
			...
			<div class="award_meter">
				...
				$ceza = $database->query("SELECT ceza FROM ceza_sistemi WHERE user_id = '$userDBID'")->fetch_assoc()['ceza'];
				...
				<div class="award_progress award_type<?php echo $ceza ?>">
			...
			<?php } if($redakte_st>0) { ?>
				...
				<div class="user_role" id="redakte_role_stat">
		...
		<div id="izin-popup" class="popup">
			...
			<div class="popup-inner">
				...
				<h3>İzin steme Sebebinizi Aşağıya Belirtin</h3>
